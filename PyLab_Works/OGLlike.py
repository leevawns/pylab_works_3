...
class ShapeCanvas ( wx.ScrolledWindow ) :
  ...
  def _OnSize ( self, event ) :
    ...
    self._Paint_Buffer = wx.Bitmap ( w, h )
    self.Refresh ()
  ...
  def _OnPopupItemSelected ( self, event ) :
    ...
    if ID == 0 :
      ...
      items = self.getSelectedShapes ()
      print ('OGLLike:OnPopup', ID, items)
      for item in items :
        if isinstance ( item, Rectangle ) :
          if item.Parent.BP_State == 0 :
            item.Parent.BP_State += 1
          item.Parent.BP_State += 1
          item.Parent.BP_State %= 4
          print ('joepi',item.Parent.BP_State)
      self.Refresh ()
    else :
      print ('OGL-Menu =', ID)
...
class Rectangle ( t_BaseShape ):
  ...
  def draw ( self, dc ):
    ...
    #************************
    # Draw BreakPoints Status
    #************************
    if self.Parent.BP_Flag or \
       self.Parent.BP_State > 1 :
      BP_Color = ( 0, wx.RED, wx.GREEN, wx.BLUE )

      if self.Parent.BP_Flag :
        color = wx.RED
      else :
        color = BP_Color [ self.Parent.BP_State ]
      r = 10
      dc.SetPen ( wx.Pen ( color, 4 ) )
      dc.DrawLine ( self._x[0] - r, self._y[0] - r,
                    self._x[1] + r, self._y[1] + r )
      dc.DrawLine ( self._x[0] - r, self._y[1] + r,
                    self._x[1] + r, self._y[0] - r )
