...
above or below the guide crosshairs to move forward or
...
for i in range(Nslabs):
    ...
    if i == 2: # Make a portal
        ...
        box(pos=(3.*w/8.0,0.75*h/2.,R),
            length=abs(-0.5*w/2),height=0.75*h,width=d, color=grey)
        box(pos=(0,0.85*h,R),
...
thairs = 0.005 # thickness of crosshairs
...
while 1:
    ...
    if scene.mouse.events:
        ...
        if m.drag or m.drop:
            roam = not roam
            if crosshairs == None:
                crosshairs = frame(pos=scene.mouse.camera+scene.forward*2., axis=scene.forward)
                curve(frame=crosshairs, pos=[(0,0,-rhairs),(0,0,rhairs)], radius=thairs, color=haircolor)
                curve(frame=crosshairs, pos=[(0,-rhairs,0),(0,rhairs,0)], radius=thairs, color=haircolor)
    ...
    if roam:
        ...
        if abs(dot(ray,scene.forward)) < maxcosine: # do something only if outside crosshairs
            ...
            crosshairs.pos = scene.mouse.camera+scene.forward*dhairs
            crosshairs.axis = scene.forward
