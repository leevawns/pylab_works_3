...
def Calc_Energy ( pat_nummer, Data_Akto, Data_SW, kCal, kCal_SW ) :
  ...
  BMR = bmr    [ pat_nummer ]
  W   = weight [ pat_nummer ] / 11538.0

  for i in range (4) :
    b = pat_slice [ 2 * i     ] 
    e = pat_slice [ 2 * i + 1 ]
    if e > b : 
      BMR_frac = BMR * ( e - b ) / ( 24 * 60 )
      kCal.append ( BMR_frac + int (round(W*Data_Akto [ b : e ].sum () ) ))
    else :
      kCal.append ( 0 ) 
    Data_Slice [ b : e ] = 9
  ...
  for i in range (4) :
    ...
    e = pat_slice [ 2 * i + 1 ]
    if e > b : 
      kCal_SW.append ( BMR + int (round(120*W*Data_SW [ b : e ].sum () ) ))
    else :
      kCal_SW.append ( 0 ) 
    Data_Slice [ b : e ] = 9
